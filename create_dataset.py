...
multivar_directory = save_directory
save_directory = "/mnt/data/sonia/cef/in/windmag"
save_name = 'windmag'

shutil.copytree(multivar_directory, save_directory, dirs_exist_ok=True)

ds = np.memmap(os.path.join(save_directory, 'slp_u_v_t_q_1940-2024_5.625deg.npy'), 
                         dtype='float32', mode='r', shape=(124188, 5, 32, 64))

windmag = np.sqrt(ds[:,1]**2 + ds[:,2]**2)
windmag = np.expand_dims(windmag, axis=1)
datafile = np.memmap(os.path.join(save_directory, 'windmag_1940-2024_5.625deg.npy'), 
                         dtype='float32', mode='w+', shape=(124188, 1, 32, 64))
datafile[:] = windmag[:]
...
statistics = {}
statistics['windmag'] = {"mean": float(windmag.mean()),"std": float(windmag.std())}
# Save the statistics to a JSON file
json_file = f'{save_directory}/norm_factors.json'
with open(json_file, 'w') as f:
    json.dump(statistics, f, indent=4)
